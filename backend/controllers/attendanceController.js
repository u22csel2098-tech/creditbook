...
const markAttendance = async (req, res) => {
  try {
    const { staffId, date, status, note, overtimeHours } = req.body;
    if (!staffId || !date || !status) return res.status(400).json({ success: false, message: 'staffId, date, status required.' });
    const validStatuses = ['present','absent','half_day','paid_leave'];
    if (!validStatuses.includes(status)) return res.status(400).json({ success: false, message: 'Invalid status.' });

    // Verify staff belongs to user
    const staff = await Staff.findOne({ _id: staffId, userId: req.user._id, isActive: true });
    ...
    const record = await Attendance.findOneAndUpdate(
      ...
      { $set: { status, note: note || '', overtimeHours: overtimeHours || 0, userId: req.user._id, staffId, date: d } },
      { new: true, upsert: true }
    );

    res.json({ success: true, message: `Marked ${status}.`, data: record });
...
const markBulkAttendance = async (req, res) => {
  try {
    const { records } = req.body; // [{ staffId, date, status, note }]
    if (!Array.isArray(records) || records.length === 0) return res.status(400).json({ success: false, message: 'records array required.' });

    const results = [];
    for (const r of records) {
      const d = new Date(r.date); d.setHours(0,0,0,0);
      const record = await Attendance.findOneAndUpdate(
        { staffId: r.staffId, userId: req.user._id, date: d },
        { $set: { status: r.status, note: r.note || '', userId: req.user._id, staffId: r.staffId, date: d } },
        { new: true, upsert: true }
      );
      results.push(record);
    }
...
const getTodayAttendance = async (req, res) => {
  try {
    ...
    const records = await Attendance.find({ userId: req.user._id, date: { $gte: today, $lt: tomorrow } }).populate('staffId','name phone salary').lean();
...
module.exports = { markAttendance, markBulkAttendance, getAttendance, getTodayAttendance, getAttendanceSummary };
