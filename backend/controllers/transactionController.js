...
const addTransaction = async (req, res) => {
  try {
    ...
    // Balance logic
    // Customer: gave = +balance (they owe more), got = -balance (they paid)
    // Supplier: gave = -balance (you paid them), got = +balance (they owe you)
    let delta = 0;
...
const getTransactions = async (req, res) => {
  try {
    const { partyId, startDate, endDate, type, limit = 100, page = 1 } = req.query;
    ...
    const [data, total] = await Promise.all([
      Transaction.find(q).populate('partyId','name type phone').sort({ date: -1 }).limit(Number(limit)).skip(skip).lean(),
