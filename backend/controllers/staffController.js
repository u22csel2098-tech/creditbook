...
const createStaff = async (req, res) => {
  try {
    const { name, phone, role, salaryType, salary, joiningDate, notes, permissions } = req.body;
...
const deleteStaff = async (req, res) => {
  try {
    const s = await Staff.findOne({ _id: req.params.id, userId: req.user._id });
...
const addPayment = async (req, res) => {
  try {
    ...
    const record = { amount: Number(amount), type: type || 'salary', description: description || '', date: date ? new Date(date) : new Date() };
    staff.paymentHistory.push(record);
    ...
    await staff.save();
    res.json({ success: true, message: 'Payment recorded.', data: staff });
  } catch (e) { res.status(500).json({ success: false, message: e.message }); }
};
...
const getSalarySummary = async (req, res) => {
  try {
    ...
    const details = staffList.map(s => {
      ...
      const earned      = parseFloat((dailyRate * daysWorked).toFixed(2));
      const due         = parseFloat(Math.max(0, earned - (s.advanceBalance || 0)).toFixed(2));
      totalDue += due;
      return { ...s, earned, due, dailyRate: parseFloat(dailyRate.toFixed(2)) };
    });
